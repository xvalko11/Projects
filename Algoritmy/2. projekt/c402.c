...
** je především inženýrská disciplína, newElement opětné objevování Ameriky nemá
...
**/
	...
	tBTNodePtr itemToInsert = malloc(sizeof(struct tBTNode));

	itemToInsert->Cont = Content;
	itemToInsert->LPtr = NULL;
	itemToInsert->RPtr = NULL;

	if ((*RootPtr)==NULL){
		*RootPtr = itemToInsert;
	}
	else if(prevTmpPtr->Cont>Content){
		prevTmpPtr->LPtr = itemToInsert;
	}
	else{
		prevTmpPtr->RPtr = itemToInsert;
	}
}
